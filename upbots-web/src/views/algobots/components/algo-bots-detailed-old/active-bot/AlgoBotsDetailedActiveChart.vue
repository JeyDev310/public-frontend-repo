<template>
  <AppCard class="flex flex-col relative w-full p-10 md:p-14 lg:p-20">
    <!-- LOADING -->
    <AppSkeleton v-if="loading || algoBotsDetailedLoading" type="lineChart" class="w-full h-full" />
...
export default defineComponent({
  name: 'AlgoBotsDetailedActiveChart',
  ...
  setup (props) {
    ...
    const { algoBotsDetailedLoading, getBotPerfSubscriptionSnapshotData } = useAlgoBotsDetailed()
    ...
    const getChartLabels = computed(() => {
      if (chartActiveTab.value === chartTabs[0].value) {
        ...
        return (getBotPerfSubscriptionSnapshotData.value as any).charts[performanceActiveFilter.value].labels
      }
    ...
    const getChartData = computed(() => {
      if (chartActiveTab.value === chartTabs[0].value) {
        // TODO FIX TYPE FOR getBotPerfSubscriptionSnapshotData //
        return [{ name: '', data: (getBotPerfSubscriptionSnapshotData.value as any).charts[performanceActiveFilter.value].data }]
      }
    ...
    const isPerformanceDataAvailable = computed(() => {
      return getBotPerfSubscriptionSnapshotData.value?.charts?.daylyChart?.data?.length &&
      getBotPerfSubscriptionSnapshotData.value?.charts?.weeklyChart?.data?.length &&
      getBotPerfSubscriptionSnapshotData.value?.charts?.monthlyChart?.data?.length
    })
