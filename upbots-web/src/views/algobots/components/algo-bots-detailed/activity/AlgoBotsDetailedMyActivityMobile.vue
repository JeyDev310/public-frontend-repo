...
export default defineComponent({
  name: 'AlgoBotsDetailedMyActivityMobile',
  ...
  setup () {
    ...
    function getProfiteTrade (auditItem: IAlgobotsSubscriptionAudits) {
    ...
    function getPerformanceFee (auditItem: IAlgobotsSubscriptionAudits) {
      const item = getProfiteTrade(auditItem)
      return Object.keys(botPerfSubscriptionCycles.value).length ? (item?.performanceFee || 0) : 0
    }

    const getBotPerfFees = computed((): number => bot.value.perfFees.percent)
