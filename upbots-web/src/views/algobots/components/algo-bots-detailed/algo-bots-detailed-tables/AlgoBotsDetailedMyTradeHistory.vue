...
export default defineComponent({
  name: 'AlgoBotsDetailedMyTradeHistory',
  ...
  setup () {
    ...
    async function closeTrading (stratType: string): Promise<void> {
      const { data } = await algoBotsService.getCurrentPerformanceFee(
        {
          botId: subscriptionBot.value.botId,
          botSubId: subscriptionBot.value.id
        }
      )
      const currentPerfee = data.result || 0
      ...
      if (isAllow) {
        ...
        try {
          ...
          await fetchPerformanceBotCyclesById(bot.value.id)
        } catch (error) {
