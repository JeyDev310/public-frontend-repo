...
export const useBuyOrder = function () {
  /* USER TRADE SETUP */
  const {
    priceSummary,
    accountTradeBalance,
  ...
  function setOrderSymbol (): void {
    ...
    state.availableSymbol = defaultPair.value.quoteCurrency
  }
  ...
  function calcTotal (): void {
    ...
    state.total = ((state.available / state.price) * state.range[0] / 100) * price
  }
  ...
  function calcOrderQuantity (): void {
    ...
    const data = (state.available / state.price) * state.range[0] / 100
  ...
  function calcRange (): void {
    const price = state.orderType.value === 'market' ? state.marketPrice : state.price
    const available = state.available / price
  ...
  function setQuantity (): void {
    ...
    state.quantity = (state.available / state.price) * state.range[0] / 100
  }
