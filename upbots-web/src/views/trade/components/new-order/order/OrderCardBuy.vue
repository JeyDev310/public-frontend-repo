<template>
  <AppCard
    class="flex flex-col items-start relative w-full gap-y-10 4xl:gap-y-20 px-20 pb-20 pt-30 md:pb-20 md:px-20"
  ...
  >
    ...
    <template v-if="availableAccounts.length">
      ...
      >
        {{ `Buy ${defaultPair?.baseCurrency ? defaultPair.baseCurrency : ''}` }}
      </AppButton>
    ...
    <AppModal v-model="createOrderModal" modal-width="600">
      <div class="w-full text-beguiling-blue dark:text-fly-a-kite space-y-30">
        <!-- ORDER TITLE -->
        ...
        <!-- ORDER DATA -->
        <div class="flex flex-col px-20 md:px-60 space-y-16">
          <!-- ACCOUNT -->
...
import { useBuyOrder } from './order-buy.composition.setup'
